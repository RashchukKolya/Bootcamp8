...
const getAllNames = arr => {
  let allNames = arr.map(function(el){
    return el.name;
  })
  return allNames;
};
...
const getUsersByEyeColor = (arr, color) => {
  let eyeColor = arr.filter(el => el.eyeColor == color)
  return eyeColor;
};
...
const getUsersByGender = (arr, gender) => {
  let userGender = arr.filter(el => el.gender == gender);
  return userGender;
};
...
const getInactiveUsers = arr => {
  let inactiveUser = arr.filter(el => el.isActive===false);
  return inactiveUser;
};
...
const getUserByEmail = (arr, email) => {
  let userEmail = arr.find(el => el.email==email);
  return userEmail;
};
...
const getUsersWithAge = (arr, min, max) => {
  let userAge = arr.filter(el => min <= el.age && el.age <= max);
  return userAge;
};
...
const getTotalBalance = arr => {
  let totalBalance = arr.reduce((acc, el) => el.balance + acc, 0);
  return totalBalance;
};
...
const getUsersByFriend = (arr, name) => {
  let usersFriends = arr.filter(el => el.friends.includes(name)).map(el => el.name);
  return usersFriends;
};
...
*/
const getAllUniqueSkills = arr => arr.map(el => el.skills)
...
const getUserNamesSortedByFriendsCount = arr => {
  let Names = arr.sort((a, b) => a.friends.length - b.friends.length).map(function(el){
    return el.name;
  })
  return Names;
};
